...
def admin_panel():
    ...
    if request.args.get('delete_task'):
        ...
        currentTask = chapters.query.filter_by(id_chapter=idChapter).first().task_list[index_delete_task - 1]
    ...
    if current_user.is_authenticated:
        if current_user.privilages == 'is_admin':
            if languages.query.first() and parts.query.first() and chapters.query.first():
                ...
                return render_template('admin-panel.html', langs=languages.query.all(),
...
def adding_part():
    if current_user.is_authenticated and current_user.privilages == 'is_admin':
        ...
        if adding_part.validate_on_submit():
            ...
            new_part = parts(languages.query.filter_by(
...
def adding_chapter():
    if current_user.is_authenticated and current_user.privilages == 'is_admin':
        ...
        if adding_chapter.validate_on_submit():
            new_chapter = chapters(parts.query.filter_by(
